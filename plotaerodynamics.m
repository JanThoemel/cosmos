...
for i=1:size(theta,2)
    for j=1:size(phi,2)
        ...
        for nb_xp=1:panels(1) 
            ...
            f1=f1+aerodynamics(panelxtheta,panelxphi)';
        end
        %% sum y-panels
        for nb_yp=1:panels(2)
            %% compute aerodynamics of 2
            f1=f1+aerodynamics(panelytheta,panelyphi)';
        end
        %% sum z-panels
        for nb_zp=1:panels(3) 
            %% compute aerodynamics of 3
            f1=f1+aerodynamics(panelztheta,panelzphi)';
        end
        f2(:,i,j)=f1;
...
figure;
        subplot(1,3,1)
        mesh(theta, phi, squeeze(f2(1,:,:))');
        grid on;xlabel('\theta_{org}');ylabel('\phi_{org}');zlabel('fx_{org}');
        subplot(1,3,2)
        mesh(theta, phi, squeeze(f2(2,:,:))');
        grid on;xlabel('\theta_{org}');ylabel('\phi_{org}');zlabel('fx_{org}');
        subplot(1,3,3)
        mesh(theta, phi, squeeze(f2(3,:,:))');
        grid on;xlabel('\theta_{org}');ylabel('\phi_{org}');zlabel('fx_{org}');
...
figure;
        ...
        grid on;xlabel('\theta_{org}');ylabel('\phi_{org}');zlabel('pitch');
        subplot(1,2,2)
        mesh(theta, phi, yaw');
        grid on;xlabel('\theta_{org}');ylabel('\phi_{org}');zlabel('yaw');
